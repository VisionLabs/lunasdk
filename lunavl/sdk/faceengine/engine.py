...
class VLFaceEngine:
    ...
    def createWarpQualityEstimator(self, launchOptions: Optional[LaunchOptions] = None) -> WarpQualityEstimator:
        ...
        """
        launchOptions = self.getLaunchOptions(launchOptions)
        if hasattr(self._faceEngine, "createSubjectiveQualityEstimator"):
            return WarpQualityEstimator(
                self._faceEngine.createSubjectiveQualityEstimator(launchOptions=launchOptions.coreLaunchOptions),
                launchOptions,
            )
        return WarpQualityEstimator(
    ...
    def createFacialHairEstimator(self, launchOptions: Optional[LaunchOptions] = None) -> FacialHairEstimator:
        """
        Create facial hair estimator.
